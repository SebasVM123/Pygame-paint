...
class Paint:
    def __init__(self):
        ...
        self.paint_button = Button(self.screen, MINT, 585, 5, 80, 40, content='BORRAR')
    ...
    def events(self):
        for event in pygame.event.get():
            ...
            #It change the width depending on whether it is eraser mode or not
            #and return to the previous color if eraser mode turns off
            if self.state == 'ERASER':
                if self.width == 2: self.width = 6
                elif self.width == 4: self.width = 12
            else:
                if self.width == 6: self.width = 2
                elif self.width == 12: self.width = 4

                if self.return_previous_color:
        ...
        while self.running:
            ...
            #TOP BAR
                ...
                if self.paint_button.isPressed(mouse_pos, mouse_pressed):
                    ...
                    #erasing is to set color to background color (white) so I store the current color to come back to
                    #it when I switch to drawing.
                    if self.color != ERASER_COLOR:
                        self.return_previous_color = 1
                        self.previous_color = self.color
